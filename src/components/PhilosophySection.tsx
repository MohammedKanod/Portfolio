  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col justify-between relative z-10">
        ...
        {/* Typographic Loop Statement */}
        <div className="mb-4 sm:mb-6">
          <span className="font-mono text-xs text-accent font-semibold uppercase tracking-widest block mb-1">
            {philosophy.eyebrow}
          </span>
          <h2 className="font-sans text-xl sm:text-3xl lg:text-5xl font-bold tracking-tightest uppercase text-ink leading-tight">
            {philosophy.loopHeadline}
          </h2>
        </div>

        {/* Principles Grid (1 col on mobile with full text, 2 col on sm+) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 pt-3 sm:pt-4 border-t border-editorial my-auto">
          {philosophy.principles.map((principle) => (
            <div
              key={principle.number}
              className="apple-glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-1.5 transition-all duration-300 hover:scale-[1.01]"
            >
              ...
              <p className="font-sans text-xs sm:text-xs text-ink-secondary leading-relaxed font-light">
